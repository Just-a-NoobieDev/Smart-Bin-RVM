...
export function AllocationReport({ data }) {
  ...
  const handleAll = async () => {
    if (data) {
      ...
      const file_name = `All-PointAllocation-Report-${moment().format(
        "YYYY-MM-DD",
      )}`;
  ...
  const handleToday = async () => {
    if (data) {
      ...
      const file_name = `PointAllocation-Report-${moment().format(
        "YYYY-MM-DD",
      )}`;
  ...
  const handleWeek = async () => {
    if (data) {
      ...
      const file_name = `PointAllocation-Report-${moment()
        .startOf("week")
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button className=" relative h-[50px] bg-[#5B801A] rounded-sm md:h-[45px] px-2 md:px-4 w-full grow outline-none">
          <div className="flex justify-center items-center gap-1 md:gap-2 text-[12px] md:text-[17px] lg:text-[15px]">
            Generate Points Allocation Report
